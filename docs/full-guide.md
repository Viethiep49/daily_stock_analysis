...
| `EMAIL_SENDER_NAME` | Tên hiển thị người gửi (mặc định: daily_stock_analysisStock Analysis Assistant) | Tùy chọn |
...
| `ANALYSIS_DELAY` | Độ trễ giữa phân tíchcổ phiếu vàthị trường (giây), tránh API rate limit, vd `10` | Tùy chọn |
| `MERGE_EMAIL_NOTIFICATION` | Gộp đẩy tincổ phiếu vàthị trườngtổng kết (mặc định false), giảm số lượng email, giảm nguy cơ spam; loại trừ lẫn nhau với `SINGLE_STOCK_NOTIFY` (không hiệu lực trong chế độtừng mã) | Tùy chọn |
| `MARKDOWN_TO_IMAGE_CHANNELS` | Kênh gửi Markdown chuyển thành ảnh (cách nhau bởi dấu phẩy): telegram,wechat,custom,email,slack; chế độtừng mã cần cấu hình đồng thời và cài công cụ chuyển ảnh | Tùy chọn |
...
| `BRAVE_API_KEYS` | [Brave Search](https://brave.com/search/api/) API (ưu tiên quyền riêng tư, tối ưuUS stocks, nhiều key cách nhau bởi dấu phẩy) | Tùy chọn |
...
| `ENABLE_CHIP_DISTRIBUTION` | Bật phân tíchphân bố chip (Actions mặc định false; cần dữ liệuchấm khi cấu hình trong Variables thành true, interface có thể không ổn định) | Tùy chọn |
...
1. Vào tab `Actions`
2. Chọn workflow `Daily Stock Analysis` ở cột trái
...
> *Lưu ý: Ít nhất phải cấu hình một trong `AIHUBMIX_KEY`, `GEMINI_API_KEY`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY` hoặc `OLLAMA_API_BASE`. Người dùng `AIHUBMIX_KEY` không cần cấu hình `OPENAI_BASE_URL`, hệ thống tự động điều chỉnh.
...
| `STOCK_GROUP_N` / `EMAIL_GROUP_N` | Nhóm cổ phiếu gửi đến email khác nhau (Issue #268), vd `STOCK_GROUP_1=VCB,FPT` ghép với `EMAIL_GROUP_1=user1@example.com` | Tùy chọn |
...
| `BRAVE_API_KEYS` | Brave Search API Key (tối ưuUS stocks) | Tùy chọn |
...
| `TICKFLOW_API_KEY` | TickFlow API Key; sau khi cấu hình,tổng kết chỉ số A-share sẽ ưu tiên thử TickFlow, nếu gói hỗ trợ tra cứu poolmã chứng khoán thì thống kê thị trường cũng ưu tiên TickFlow | - | Tùy chọn |
...
| `ENABLE_CHIP_DISTRIBUTION` | Bật phân tíchphân bố chip (interface không ổn định, khuyến nghị tắt khi deploy cloud). Người dùng GitHub Actions cần đặt `ENABLE_CHIP_DISTRIBUTION=true` trong Repository Variables để bật; workflow mặc định tắt. | `true` | Tùy chọn |
| `ENABLE_EASTMONEY_PATCH` | PatchAPI Đông Phương: khiAPI Đông Phương liên tục thất bại (vd RemoteDisconnected, kết nối bị đóng) khuyến nghị đặt `true`, tiêm token NID và User-Agent ngẫu nhiên để giảm xác suất bị giới hạn | `false` | Tùy chọn |
...
| `ENABLE_FUNDAMENTAL_PIPELINE` | Master switch cho tổng hợp dữ liệu cơ bản; tắt thì chỉ trả về khối `not_supported`, không thay đổi chuỗi phân tích gốc | `true` | Tùy chọn |
| `FUNDAMENTAL_STAGE_TIMEOUT_SECONDS` | Ngân sách độ trễ giai đoạncơ bản (giây) | `1.5` | Tùy chọn |
| `FUNDAMENTAL_FETCH_TIMEOUT_SECONDS` | Timeout gọi nguồn đơn (giây) | `0.8` | Tùy chọn |
| `FUNDAMENTAL_RETRY_MAX` | Số lần thử lại nguồncơ bản (bao gồm lần đầu) | `1` | Tùy chọn |
| `FUNDAMENTAL_CACHE_TTL_SECONDS` | TTL cachetổng hợpcơ bản (giây), cache ngắn giảm kéo lặp | `120` | Tùy chọn |
| `FUNDAMENTAL_CACHE_MAX_ENTRIES` | Số entry tối đa cachecơ bản (loại bỏ theo thời gian trong TTL) | `256` | Tùy chọn |

> Giải thích hành vi:
> - A-share:tổng hợp trả về theo `valuation/growth/earnings/institution/capital_flow/dragon_tiger/boards`;
> - ETF: trả về mục có được, năng lực thiếu đánh dấu `not_supported`, tổng thể không ảnh hưởng quy trình gốc;
> -US stocks/HK stocks: trả về khối dự phòng `not_supported`;
> - Bất kỳ ngoại lệ nào đều fail-open, chỉ ghi lỗi, không ảnh hưởng chuỗi chính kỹ thuật/tin tức/chấm.
> - Sau khi cấu hình `TICKFLOW_API_KEY`, chỉtổng kết chỉ số A-share sẽ ưu tiên thử TickFlow cho chỉ số chính; nếu gói hiện tại hỗ trợ tra cứu poolmã chứng khoán thì thống kê thị trường cũng ưu tiên TickFlow. Chuỗicổ phiếu và realtime quote không thay đổi ưu tiên.
> - Năng lực TickFlow phân lớp theo quyền gói: gói quyền hạn chế vẫn có thể dùng tra cứu chỉ số chính; gói hỗ trợ poolmã chứng khoán `CN_Equity_A` mới bật thống kê thị trường TickFlow.
> - Quickstart chính thức đã document hóa `quotes.get(universes=["CN_Equity_A"])`, nhưng smoke test online xác nhận thêm: `TICKFLOW_API_KEY` không đồng nghĩa có quyền này, và `quotes.get(symbols=[...])` có giới hạn sốmã chứng khoán mỗi lần.
> - TickFlow thực tế trả về `change_pct` / `amplitude` là tỷ lệ; hệ thống đã chuyển đổi thống nhất thành giá trị phần trăm tại lớp tiếp nhận, đảm bảo nhất quán ngữ nghĩa trường với nguồn dữ liệu hiện có.
> - Hợp đồng trường:
>   - `fundamental_context.belong_boards` = danh sáchngành liên quancổ phiếu (hiện chỉ A-share; không có dữ liệu thì `[]`);
>   - `fundamental_context.boards.data` = `sector_rankings` (bảng xếp hạngngành, cấu trúc `{top, bottom}`);
...
>   - `fundamental_context.earnings.data.dividend` = chỉ số chia cổ tức (chỉphạm vi cổ tức tiền mặt trước thuế, gồm `events`, `ttm_cash_dividend_per_share`, `ttm_dividend_yield_pct`);
>   - `get_stock_info.belong_boards` = danh sáchngành thuộccổ phiếu;
...
>   - `AnalysisReport.details.belong_boards` = danh sáchngành liên quan trong chi tiết báo cáo có cấu trúc;
>   - `AnalysisReport.details.sector_rankings` = bảng xếp hạng ngành trong chi tiết báo cáo có cấu trúc (dùng cho liên kết ngành ở frontend).
> - Thứ tự nguồn dữ liệu bảng xếp hạngngành: nhất quán với priority toàn cục.
...
> - `FUNDAMENTAL_STAGE_TIMEOUT_SECONDS=1.5` là ngân sách mục tiêu cho giai đoạncơ bản mới, không phải SLA cứng nghiêm ngặt.
> - Nếu cần SLA cứng, vui lòng nâng cấp lên thực thi cách ly tiến trình con và buộckết thúc bắt buộc khi timeout ở phiên bản sau.
...
| `TRUST_X_FORWARDED_FOR` | Đặt `true` khi deploy sau reverse proxy đáng tin 1 lớp, lấy giá trị ngoài cùng bên phải của `X-Forwarded-For` làm IP client thực (dùng cho rate limit đăng nhập...); giữ `false` khi kết nối trực tiếp internet để chống giả mạo. Với multi-proxy/CDN, key rate limit có thểtrở về hóa thành IP edge proxy, cần đánh giá thêm | `false` |
| `MAX_WORKERS` | Số luồngđồng thời | `3` |
| `MARKET_REVIEW_ENABLED` | Bậttổng kết thị trường | `true` |
| `MARKET_REVIEW_REGION` | Thị trườngtổng kết: cn(A-share), us(US stocks), both(cả hai), us phù hợp người dùng chỉ quan tâmUS stocks | `cn` |
...
```bash
python main.py                        # Phân tích đầy đủ (cổ phiếu +tổng kết thị trường)
python main.py --market-review        # Chỉtổng kết thị trường
python main.py --no-market-review     # Chỉ phân tíchcổ phiếu
...
python main.py --workers 5            # Chỉ định sốđồng thời
```
...
Mặc định phán đoán dựa trên thị trường cổ phiếu tự chọn (A-share / HK /US stocks) và `MARKET_REVIEW_REGION`:
- Sử dụng `exchange-calendars` để phân biệt lịch giao dịch riêng của A-share / HK /US stocks (bao gồm ngày lễ)
...
Nếu không muốn dùng process thường trú, có thể dùng Cron của hệ thống:
...
Cấu hình `STOCK_GROUP_N` và `EMAIL_GROUP_N` để gửi báo cáo nhóm cổ phiếu khác nhau đến email khác nhau, ví dụ nhiều người chia sẻ phân tích không ảnh hưởng lẫn nhau.tổng kết thị trường sẽ gửi đến tất cả email đã cấu hình.
...
Trong chế độ đẩy từng mã (`SINGLE_STOCK_NOTIFY=true`), nếu muốn kênh như Telegram gửi dưới dạng ảnh, cần cấu hình đồng thời `MARKDOWN_TO_IMAGE_CHANNELS=telegram` và cài công cụ chuyển ảnh (wkhtmltopdf hoặc markdown-to-file). Tóm tắt nhật kýcổ phiếu cũng hỗ trợ chuyển ảnh, không cần cấu hình thêm.
...
### YFinance
- Miễn phí, không cần cấu hình
- Hỗ trợUS stocks/HK stocks
- Dữ liệu lịch sử và realtime US stocks đều thống nhất dùng YFinance, tránh chỉ số kỹ thuật sai do akshare US stocks điều chỉnh giá bất thường

### Xử Lý KhiAPI Đông Phương Liên Tục Thất Bại
...
Đối với ETF theo dõi chỉ số và US stock index ETF (như VOO, QQQ, SPY, SPX, DJI, IXIC), phân tích chỉ tập trung vào**xu hướng chỉ số, sai số theo dõi, thanh khoản thị trường**, không đưa vào rủi ro cấp công ty của nhà quản lý/phát hành quỹ (kiện tụng, danh tiếng, thay đổi lãnh đạo...). Cảnh báo rủi ro và dự kiến hiệu suất đều dựa trên biểu hiện tổng thể của cổ phiếu thành phần chỉ số, tránh nhầm lẫn tin tức quỹ thànhtin xấu củamã chứng khoán. Xem Issue #274.
...
**Cơ chế 2 lớp**: Luân phiên nhiều Key cho cùng mô hình (Router) vàfallback cross-model (Fallback) tách biệt độc lập, không ảnh hưởng lẫn nhau.

**Ví dụ cấu hình đa Key +fallback cross-model**:
...
#fallback cross-model: khi tất cả Key mô hình chính đều thất bại, thử Claude → GPT theo thứ tự
...
**Mô hình Vision (ảnh trích mã cổ phiếu)**: Xem chi tiết [Hướng Dẫn Cấu Hình LLM - Vision](LLM_CONFIG_GUIDE.md#vision-model).
...
- **Cache phân tích tên AkShare**: Khi dùng fallback online AkShare để phân tích tên→mã, kết quả cache 1 giờ (TTL), tránh requestthường xuyên; tự động refresh khi gọi lần đầu hoặc cache hết hạn.
- **Tên cột CSV/Excel**: Hỗ trợ `code`, `mã cổ phiếu`, `name`, `tên cổ phiếu`... (không phân biệt hoa thường); không có header thì mặc định cột 1 là mã, cột 2 là tên.
...
Module backtest tự động xác minhsau thực tế đối với bản ghi phân tích AI lịch sử, đánh giá độ chính xác của khuyến nghị phân tích.
...
| `BACKTEST_NEUTRAL_BAND_PCT` | `2.0` | Ngưỡng vùng trung tính (%), ±2% coi làsideway |
...
> Lưu ý: `POST /api/v1/analysis/analyze` khi `async_mode=false` chỉ hỗ trợ 1 cổ phiếu; `stock_codes` batch cần dùng `async_mode=true`. Phản hồi async `202` trả về `task_id` chocổ phiếu, trả về cấu trúc tổng hợp `accepted` / `duplicates` cho batch.
...
|US stocks | 1-5 chữ cái (có thể hậu tố .X) | `AAPL`, `TSLA`, `BRK.B` |
| US stock index | SPX/DJI/IXIC... | `SPX`, `DJI`, `NASDAQ`, `VIX` |
...
- Trang `/portfolio` giờ bao gồm:
  ...
  - khi `PORTFOLIO_FX_UPDATE_ENABLED=false`, API refresh giờ trả về trạng thái disabled rõ ràng và trang sẽ hiển thị thông báo "tỷ giá bị tắt" thay vì "không có cặp tỷ giá cần refresh"
