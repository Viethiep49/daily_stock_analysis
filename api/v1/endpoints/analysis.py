...
3. Provide GET /api/v1/analysis/tasks to get task list
4. Provide GET /api/v1/analysis/tasks/stream SSE real-time push endpoint

Features:
- Async task queue: analysis tasks run asynchronously without blocking requests
- Duplicate submission prevention: returns 409 when the same stock code is already being analyzed
- SSE real-time push: notifies the frontend of task status changes in real time
"""
...
def _invalid_analysis_input_error() -> HTTPException:
    return HTTPException(
        ...
        detail={
            ...
            "message": "Please enter a valid stock code or stock name",
        },
    )
...
@router.post(
    ...
    responses={
        200: {"description": "Analysis complete (sync mode)", "model": AnalysisResultResponse},
        202: {
            "description": "Analysis task accepted (async mode)",
            "model": Union[TaskAccepted, BatchTaskAcceptedResponse],
        },
        400: {"description": "Invalid request parameters", "model": ErrorResponse},
        409: {"description": "Stock is already being analyzed, duplicate submission rejected", "model": DuplicateTaskErrorResponse},
        500: {"description": "Analysis failed", "model": ErrorResponse},
    },
    summary="Trigger stock analysis",
    description="Start an AI intelligent analysis task, supporting both sync and async modes. In async mode, duplicate submissions for the same stock code are not allowed."
)
def trigger_analysis(
    ...
    Start an AI intelligent analysis task, supporting single or batch stock analysis.

    Flow:
    1. Validate request parameters
    2. Async mode: check duplicates -> submit to task queue -> return 202
    3. Sync mode: execute analysis directly -> return 200
    ...
    Returns:
        AnalysisResultResponse: Analysis result (sync mode)
        TaskAccepted | BatchTaskAcceptedResponse: Task accepted (async mode, returns 202)
    ...
    if not stock_codes:
        raise HTTPException(
            ...
            detail={
                "error": "validation_error",
                "message": "Must provide stock_code or stock_codes parameter"
            }
        )
    ...
    if len(stock_codes) > MAX_BATCH_SIZE:
        raise HTTPException(
            ...
            detail={
                "error": "validation_error",
                "message": f"A single analysis request supports at most {MAX_BATCH_SIZE} stocks"
            }
        )
    ...
    if not stock_codes:
        raise HTTPException(
            ...
            detail={
                "error": "validation_error",
                "message": "Stock code cannot be empty or contain only whitespace"
            }
        )
    ...
    if not request.async_mode:
        if len(stock_codes) > 1:
            raise HTTPException(
                ...
                detail={
                    "error": "validation_error",
                    "message": "Sync mode only supports single-stock analysis; use async_mode=true for batch analysis"
                }
            )
...
def _handle_async_analysis_batch(
    ...
    # Single stock rejected: maintain 409 compatibility
    ...
    # Single stock accepted: maintain original response format compatibility
    ...
    # Batch: return summary result
    batch_response = BatchTaskAcceptedResponse(
        accepted=accepted,
        duplicates=duplicates,
        message=f"Submitted {len(accepted)} task(s), {len(duplicates)} duplicate(s) skipped",
    )
    return JSONResponse(
        status_code=202,
...
def _handle_sync_analysis(
    ...
    Executes analysis directly and returns the result after completion.
    """
    import uuid
        ...
        if result is None:
            raise HTTPException(
                ...
                detail={
                    ...
                    "message": f"Failed to analyze stock {stock_code}"
                }
            )
...
@router.get(
    ...
    summary="Get analysis task list",
    description="Get all current analysis tasks, filterable by status"
)
def get_task_list(
    status: Optional[str] = Query(
        None,
        description="Filter by status: pending, processing, completed, failed (comma-separated multiple values supported)"
    ),
    limit: int = Query(20, description="Return count limit", ge=1, le=100),
) -> TaskListResponse:
    """
    Get analysis task list
...
@router.get(
    ...
    description="Push task status changes in real time via Server-Sent Events"
)
...
# ============================================================
# GET /status/{task_id} - Query single task status
# ============================================================
...
@router.get(
    ...
    summary="Query analysis task status",
    description="Query the status of a single task by task_id"
)
def get_analysis_status(task_id: str) -> TaskStatus:
    """
    Query analysis task status

    Looks up the task queue first; if not found, queries the database for historical records.
...
def _build_analysis_report(
    ...
    """
    Build an analysis report conforming to the API specification.
