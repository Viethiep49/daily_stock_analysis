...
Usage:
    ...
    /ask 600519 chan_theory_analysis     -> Parse skill from message
...
class AskCommand(BotCommand):
    ...
    def description(self) -> str:
        return "Analyze stocks using Agent skills"

    @property
    def usage(self) -> str:
        return "/ask <stock_code[,code2,...]> [skill_name]"
    ...
    def _validate_single_code(self, code: str) -> Optional[str]:
        ...
        if not (is_a_stock or is_hk_stock or is_us_stock):
            return f"Invalid stock code: {normalized} (A-share: 6 digits / HK: HK+5 digits / US: 1-5 letters)"
        return None

    def validate_args(self, args: List[str]) -> Optional[str]:
        """Validate arguments."""
        if not args:
            return "Please enter a stock code. Usage: /ask <stock_code[,code2,...]> [skill_name]"

        raw_code_str, _ = self._merge_code_args(args)
        codes = self._parse_stock_codes(raw_code_str)
        if not codes:
            return "Please enter at least one valid stock code"
        ...
        if len(codes) > 5:
            return "Maximum 5 stocks per analysis"
    ...
    def _build_user_message(stock_code: str, skill_id: str, skill_text: str) -> str:
        user_msg = f"Please analyze stock {stock_code}"
        if skill_id:
            user_msg = f"Please use {skill_id} skill to analyze stock {stock_code}"
        if skill_text:
            user_msg = f"Please analyze stock {stock_code}, {skill_text}"
        return user_msg
    ...
    def execute(self, message: BotMessage, args: List[str]) -> BotResponse:
        ...
        if not config.agent_mode:
            return BotResponse.text_response(
                "⚠️ Agent mode is not enabled. Cannot use stock query.\nPlease set `AGENT_MODE=true` in your config."
            )
    ...
    def _analyze_single(
        ...
        try:
            ...
            return BotResponse.text_response(f"⚠️ Stock query execution error: {str(exc)}")
    ...
    def _analyze_multi(
        ...
        try:
            for future in as_completed(future_map, timeout=overall_timeout_s):
                try:
                    ...
                    errors[code] = f"Execution error: {exc}"
            ...
            for future, code in future_map.items():
                ...
                if future.done():
                    ...
                    errors[code] = "Analysis timed out (did not complete within 150 seconds)"
        finally:
            pool.shutdown(wait=False, cancel_futures=True)

        for code in codes:
            if code not in results and code not in errors:
                errors[code] = "Analysis timed out"

        parts = [f"📊 **Multi-stock Comparison Analysis** | Skill: {skill_name}", f"{'─' * 30}", ""]
        ...
        if len(results) >= 2:
            parts.append("| Stock | Signal | Confidence | Summary |")
            parts.append("|-------|--------|------------|---------|")
            for code in codes:
                if code in results:
                    item = results[code]
                    ...
                    summary_line = str(item.get("summary") or "Analysis complete").replace("|", "/")[:80]
    ...
    def _extract_summary(stock_code: str, dashboard: Optional[Dict[str, Any]], raw_content: str) -> str:
        ...
        for line in raw_content.splitlines():
            ...
            if stripped and len(stripped) > 4 and not stripped.startswith(("{", "}", "\"")):
                return stripped[:120]
        return f"{stock_code} analysis complete"
    ...
    def _format_stock_result(stock_code: str, dashboard: Optional[Dict[str, Any]], raw_content: str) -> str:
        if not isinstance(dashboard, dict):
            content = raw_content
            if len(content) > 800:
                content = content[:800] + "\n... (truncated, please query individually for full analysis)"
            return content
        ...
        if isinstance(decision, str):
            lines.append(
                ...
                + (f" | **Trend**: {trend}" if isinstance(trend, str) and trend.strip() else "")
            )
